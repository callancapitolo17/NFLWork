...
monte_carlo_best_of_7 <- function() {
  ...
  home_court_order <- c(favorite, favorite, underdog, underdog,favorite)
  ...
  simulate_series <- function(favorite_wins, underdog_wins) {
    for (game in home_court_order) {
      ...
      if (favorite_wins == 3) return(paste(favorite, "3", underdog, underdog_wins))
      if (underdog_wins == 3) return(paste(underdog, "3", favorite, favorite_wins))
    }
    return(ifelse(favorite_wins == 3, paste(favorite, "3", underdog, underdog_wins), paste(underdog, "3", favorite, favorite_wins)))
  }
  ...
  favorite_prob <- mean(grepl(paste0(favorite, " 3"), results))
  underdog_prob <- mean(grepl(paste0(underdog, " 3"), results))
