...
- **Commit structure**: per-phase commits, each ships with its own tests in the same commit. **Critical ordering rule**: the table rename (`draft_odds` → `kalshi_odds`) and ALL its consumer updates ship in one atomic commit, otherwise the existing dashboard breaks between commits.
