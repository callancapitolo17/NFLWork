...
- A league snapshot is `content.unabated.com/markets/v2/league/{id}/odds.json?t=<30s bucket>`, gzipped JSON: `odds` keyed `lg{league}:pt{period}:pregame` → rows `{eventId, eventStart, betTypeId, eventTeams[{rotationNumber, …}], sides: {"si0:tid6": {"ms4": {points, americanPrice, bacr, ge, liquidity, sequenceNumber, alternateLines[]}}}}`; `teams` `{id: {name, abbreviation}}`; `marketSources` `[{id, name, isActive, isEnabledForGameOdds, hasLiquidity}]`. Sizes today: NFL 3.3 MB, CFB 8.8 MB, MLB 1.3 MB compressed; NFL 55 MB and MLB 26 MB once unzipped.
- The fair, `bacr` (a whole American price), sits on every book's line, and every book at the same event, period, bet type, side and number carries the same value: 561 of 561 NFL groups and 275 of 275 MLB groups agreed. The "Unabated" source (id 49) is priced exactly at `bacr`. There is no separate consensus row, and books sitting at different numbers give the fair at each of those numbers.
- The fair is missing on part of the board. On games within 30 h (MLB) or 80 to 100 h (football), full-game moneyline, spread and total carry it on 15 of 15 NFL, 15 of 22 MLB and 60 of 118 CFB games; the 58 CFB games without it are thin (median 6 books, against 31). Football first-half and quarter totals carry it; the second half, and every half or quarter spread and moneyline, do not. MLB first five carries it on most games; single innings and the other partial-game periods do not.
- Pinnacle (source 70) is listed in `marketSources` but has zero lines on the NFL, CFB and MLB boards. Circa (sources 6 and 68) is on most full-game rows and almost no period rows.
- `eventStart` has no zone suffix and is UTC: a 4:25 PM ET NFL kickoff reads `20:25:00`.
...
- Clocks: `eventStart` is zone-less UTC from the feed and is parsed as UTC, closes stamp UTC; the BFA plus-7-hours clock affects only the bet side and is already handled.
