...
def test_single_leg_with_structure_odds_prices_without_any_price_call():
    ...
    # sides, the fair is the exact 2-cell devig and the book's SGP price
    # endpoint is never hit (several books refuse 1-selection sets).
    # Opt-in only: the enabling caller (kalshi_rfi) also owns structure
    # freshness via structure_ttl_sec.
    calls = []
    ...
    svc = SGPService(
        books=("novig",),
        on_demand_hooks={"novig": _hooks(_resolved(1, singles=True), price)},
...
def test_single_leg_fast_path_is_off_by_default_sgp_pipeline_untouched():
    # Default single_leg_structure_fair=False: a 1-leg set prices exactly as
    # before the fast path existed (wire price calls via Route A) — the
    # MM/taker SGP pipeline must be byte-for-byte unaffected.
    calls = []

    def price(client, refs, event):
        calls.append(list(refs))
        (ref,) = refs
        return 1.90 if ref.startswith("r") else 1.95

    svc = _svc("novig", _hooks(_resolved(1, singles=True), price))
    res = svc.price_on_demand("novig", GAME, _legs(1))

    assert res is not None
    assert res.route == "partition"          # wire-priced, not structure odds
    assert len(calls) == 2                   # target + opposite cell


def test_single_leg_fast_path_rejects_crossed_structure_pair():
    # devig_partition's [1.0, 1.25] overround gate: a crossed pair (implied
    # sum < 1, a feed glitch) must NOT devig silently into the consensus —
    # it falls through to the wire routes, which here decline cleanly.
    def price(client, refs, event):
        return None                          # book refuses 1-selection sets

    crossed = [ResolvedLeg(ref="r0", opposite_ref="o0",
                           single_decimal=2.3, opposite_decimal=2.3)]
    svc = SGPService(
        books=("novig",),
        on_demand_hooks={"novig": _hooks(crossed, price)},
        single_leg_structure_fair=True)
    assert svc.price_on_demand("novig", GAME, _legs(1)) is None
...
def test_single_leg_without_structure_odds_falls_through_to_route_a():
    ...
    svc = SGPService(
        books=("draftkings",),
        on_demand_hooks={"draftkings": _hooks(_resolved(1, singles=False),
                                              price)},
        single_leg_structure_fair=True)
