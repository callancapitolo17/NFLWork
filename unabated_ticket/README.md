...
main lines (board, book, bet type, period, edge, start, line age,
liquidity) plus one of their own — most alt "edges" are deep longshots (live 2026-09-11 the
...
**Min liquidity $** (default 100) hides every line that reports less than
that resting, i.e. exchanges, main lines and alts alike (Kalshi's median
alt depth was $129, Novig's $250); books with no figure pass, 0 = off.
Until 2026-09-23 it gated alts only, so a Novig Portland Fire +809 main
moneyline with $17 behind it listed with "add $32.58". Liquidity comes
from the league snapshot only (the changes stream carries none), so it can
lag a price move by up to a snapshot cycle. Sort by edge,
stake or start time. Settings (sports, periods, bets, books, minimum edge,
minimum suggested bet, max line age, min liquidity, sort) persist in `chrome.storage.local` under `edges` (sports
