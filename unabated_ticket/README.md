...
- **Same market, another period (1H total with FG total): worst case.**
  Unabated says nothing about how two periods move together and no correlation
  is estimated (user decision 2026-09-18), so each period's rows are sorted by
  P&L and paired by cumulative probability — bad with bad, good with good.
  Hold 1H Over $300 at +110 (55%), new FG Over +120 (50%), K $8,000: $667
  alone, $408 worst case (the measured NFL link 0.69 would give $530; $408
  keeps 95% of the growth). A hedge across periods gets no credit.
...
| `related_opposite` | same market, other direction, another period | `other side` (red) | yes, worst case |
...
`game · not sized` for another market, `no fair at 36.5`, `parlay leg`,
...
+3.5 held adds $0; a 1H over held sizes the FG over at $408.39; the same
