...
`ON_DEMAND_CONCURRENCY_<BOOK>` to 1 **and restart the bot** — widths are
read at import and frozen into each book's gate the first time it is used,
so an export alone changes nothing in a running process. A background structure-warming pass
...
| `SGP_BOOKS` | `draftkings,fanduel,novig,betmgm,caesars` | Books the maker's SGP service prices at all — on-demand flights and structure warming. **ProphetX is out (2026-09-15)**: `www.prophetx.co` has served a CloudFront 403 to this IP for the whole site since 2026-09-01 and the 120 s warming pass alone was failing 6k–42k times a day against it, the same self-funded WAF block #90 measured on Caesars; those doomed requests kept the block open for every other PX use on this machine (#117's content-script recon included). Add `prophetx` back once the block has cleared and #91's rate cap exists. Read at startup — a change needs a restart |
