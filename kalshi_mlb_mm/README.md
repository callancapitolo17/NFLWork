...
- **F5-winner (`KXMLBF5`) tie-adjusted pricing — issue #86.** Kalshi's F5 winner family is 3-way ({TEAM_A, TEAM_B, TIE} tickers per game) and the team markets are **unconditional — ties lose** ("all team strikes will resolve to No", live rules 2026-08-12). Every book that prices F5 ML quotes a **conditional push-2-way** number instead (live recon implied sums: DK 1.059–1.069, FD 1.0544, MGM 1.0598, Novig 1.0100; CZR verified by family via its "1st 3 Innings Money Line" at 1.0623 — its F5 board posts near lineups). Quoting that conditional number unconverted would overprice Kalshi YES by ~P(tie) ≈ 12–15% — a craftable pick-off. The conversion is the exact identity `P(combo) = P(combo | not tied) × (1 − P(tie))` (valid inside multi-leg combos with the *marginal* P(tie), because "team leads after 5" ⊂ "not tied"), implemented in `kalshi_common/fair_value.py::f5_winner_fair_from_book` and applied at the **engine's `lookup` choke point** so the quote tick, confirm last-look, and book-drift breaker all see converted fairs *before* the σ_z dispersion gate. **Semantics live in the book adapters**: each on-demand hook dict labels `f5_ml_semantics` (`"push_2way"` at DK/FD/MGM/NV/CZR; PX deliberately unlabeled — zero F5 markets on its board — so it fails closed if it ever re-lists), and an unlabeled book never prices an F5-winner set (zero wire calls). **P(tie) anchor**: FD's "First 5 Innings Result" and MGM's "1st 5 Innings - 1X2" 3-way markets ride the same structure fetch (zero extra wire calls) as `result3` decimals; `f5_tie_prob` deviggs them (probit, `tie_prob_from_three_way`) and each flight converts every book with the **median** of its landed anchors (live anchors agreed within ~0.8pt of Kalshi's own TIE mid). Zero anchors landed → `lookup` serves `{}` → declined `too_few_books`, never a raw conditional fair. **TIE-side legs stay unpriceable** (no book prices a bare F5-tie leg; declined `out_of_scope_f5_tie_leg`), and home+away F5-winner pairs die on the duplicate-market guard. Firehose provenance: `on_demand_result` carries `p_tie_used` plus per-book `f5_semantics`/`p_tie_book` (per-book `fair` stays RAW conditional for auditability), and `quote_priced.live_games` carries `p_tie_used` per game — the audit trail for the first settled fills.
