...
| `MAX_RFQ_AGE_SEC` | `600` | Skip RFQs older than this before any work. An RFQ resting 10+ min un-quoted is near expiry or already picked over — quoting late is adverse selection, and classifying it burns lap budget. Missing/unparseable `created_ts` passes (fail-open); skips are counted in the periodic pass-summary log, not per-RFQ decision rows |
