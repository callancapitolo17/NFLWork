...
**Per-book fetch health.** Every SGP fetch (sweep and on-demand) writes one row to `sgp_fetch_health` in the bot's MARKET DB — book, path, outcome (`ok`/`empty`/`transport_error`/`timeout`/`error`), duration, error class, and the #35 counter snapshot. Same buffer-and-swallow discipline as the research firehose: `SGPService.flush_health()` runs once per tick and a locked DB costs history, never a quote. 30-day retention. Schema, caveats and the ready-made queries (`kalshi_common/fetch_health_queries.sql`) are documented in `mlb_sgp/README.md` → "Fetch-health history". **Always group by `path`** — the counter units differ between sweep and on-demand.

**Run-time book-health alerting.** `check_book_health()` runs every tick off in-memory streaks fed by the same call sites that write those rows: `BOOK_ALERT_STREAK` (default 3) consecutive non-`ok`/non-`empty` outcomes on one (book, path) → one ERROR log + one macOS notification per incident, re-armed only when the book answers again; healthy books below `MIN_BOOK_COUNT_FOR_BLEND` (the blend gate itself) → "quoting effectively blind". `empty` is a healthy answer and a book skipped by `min_refresh_sec` is not a failure. Full semantics, the known silent-parser gap, and why this reads memory rather than the DB: `mlb_sgp/README.md` → "Run-time book-health alerting". Knobs: `BOOK_ALERT_ENABLED`, `BOOK_ALERT_STREAK`, `BOOK_ALERT_PATHS`.
