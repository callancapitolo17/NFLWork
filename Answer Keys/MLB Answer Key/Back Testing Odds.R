...
flat_odds <- clean_history_df %>%
  ...
  mutate(
    ...
    home_odds = ifelse(market_type == "totals",NA, ifelse(market_type == "moneyline" & home_team == outcome_name_1  , closing_odds_1,closing_odds_2)),
    away_odds = ifelse(market_type == "totals",NA, ifelse(market_type == "moneyline" & away_team == outcome_name_1, closing_odds_1,closing_odds_2)),
  ) %>% 
  group_by(
...
joined_dt <- inning_dt[odds_dt, 
                       on = .(home_team, away_team, game_date, game_start_time = commence_time), 
                       roll = "nearest",  # closest time (could use "forward" or "backward" for direction)
                       nomatch = 0L
] #check match. Unsure how event works
