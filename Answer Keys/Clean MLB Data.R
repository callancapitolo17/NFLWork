...
history_df <- fromJSON(content(res, "text"), flatten = TRUE) %>%
  as_tibble() %>%
  # unwind each nested list-column in turn
  unnest_longer(bookmakers) %>%
  unnest_wider(bookmakers, names_sep = ".") %>%
  unnest_longer(bookmakers.markets) %>%
  unnest_wider(bookmakers.markets,   names_sep = ".") %>%
  unnest_longer(bookmakers.markets.outcomes) %>%
  unnest_wider(bookmakers.markets.outcomes, names_sep = ".") %>%
  # pick & rename the bits we care about
  transmute(
    game_id             = id,
    commence_time       = ymd_hms(commence_time, tz="UTC"),
    home_team, away_team,
    bookmaker_key       = bookmakers.key,
    bookmaker_title     = bookmakers.title,
    bookmaker_update    = ymd_hms(bookmakers.last_update, tz="UTC"),
    market_key          = bookmakers.markets.key,
    market_update       = ymd_hms(bookmakers.markets.last_update, tz="UTC"),
    outcome_name        = bookmakers.markets.outcomes.name,
    closing_odds        = bookmakers.markets.outcomes.price,
    outcome_last_update = ymd_hms(bookmakers.markets.outcomes.last_update, tz="UTC")
  )
...
write.csv(flat_odds, "flatodd.csv")  
