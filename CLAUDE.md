...
- **Unabated-anchored Kalshi edge engine** (`unabated_edge/`) — sport-agnostic dry-run engine that anchors fair value on Unabated sharp-book prices, deviggs via probit, maps events to open Kalshi YES markets, and flags +EV opportunities using fractional Kelly sizing. Soccer (World Cup moneylines, adapter `sports/soccer.py`) ships first; adding a sport is one adapter file + one registry line. Writes `line_snapshots` + `flagged_edges` to `unabated_edge_market.duckdb` and a research firehose to `unabated_edge_research.duckdb`. Reuses `kalshi_common/` for fee math and the Kalshi REST client. Entry point: `python -m unabated_edge.runner`. See `unabated_edge/README.md`.
