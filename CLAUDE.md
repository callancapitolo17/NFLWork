...
- **Unabated Ticket** (`unabated_ticket/`) — Chrome MV3 side-panel extension (plain JS, no build). Click an Unabated price to build a one-at-a-time ticket with a quarter-Kelly stake and a line-moved watcher. **Edges tab** scans Unabated's public league snapshots + changes stream for ML/spread/total edges (alt lines behind a toggle, grouped by market), with book filters and opt-in Chrome-notification alerts. No overlay, no tracking, no order placement. Load unpacked from `unabated_ticket/extension`. See `unabated_ticket/README.md`.
- **One-sided Kalshi YRFI maker** (`kalshi_rfi/`) — rests YES bids only on `KXMLBRFI` at book-consensus fair − 3¢ (single-leg structure-devig fast path; ~3 books price). Worst-case-dollar caps per game/day with a hard pre-wire backstop, per-cycle order reconciliation against Kalshi (state keyed on order id, exchange-shard-aware cancels with verified 404s), doubleheaders fail-closed. Shadow by default (`RFI_MODE=live` + `RFI_LIVE_ACK=1` to go live). Writes `kalshi_rfi/kalshi_rfi.duckdb`. See `kalshi_rfi/README.md`.
