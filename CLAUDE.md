...
- **DraftKings price sidecar** (`dk_price_sidecar/`) — local loopback service (port 8095, `dk_price_sidecar/run.sh`) owning a real minimized Chrome that makes DK's `calculateBets` SGP price call one at a time; exists because since ~2026-08-20 Akamai denies that endpoint to any non-browser client and to concurrent bursts (issue #102 — curl_cffi fails on every profile, headless Chrome fails, IP/auth/cookies/headers all ruled out by controlled runs). Bots opt in with `DK_PRICE_TRANSPORT=sidecar` (default `http`); a 403/503/unreachable sidecar is counted as a DK transport error, never a parser failure. Same-game combos only — cross-game pricing never needed it. See `dk_price_sidecar/README.md`.
