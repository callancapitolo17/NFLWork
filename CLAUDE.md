...
- **Autonomous Kalshi MLB MM (maker) bot** (`kalshi_mlb_mm/`) — independent maker daemon that quotes **arbitrary MLB combos** (moneyline `KXMLBGAME` + spread + total, single- or cross-game, N legs) at a fixed 5% ROI margin by listening for others' RFQs. Pricing groups legs by game, prices each game's sub-combo to a book-consensus fair (4-cell SGP grid for same-game spread+total; 2-way Odds API singles devig for single legs), and multiplies independent games (`kalshi_mlb_mm/combo_pricer.py`). REST polling behind `RFQSource`/`QuoteGateway` interfaces (WebSocket swap is a one-adapter change); own market DB `kalshi_mlb_mm/kalshi_mlb_mm_market.duckdb` holds the SGP spread×total grid (`mlb_sgp_odds`) **and** moneyline/spread/total singles (`mlb_singles_odds`, one Odds API request/cycle, all books); shares pricing math with the taker via `kalshi_common/`. Player props (`KXMLBHIT`/`TB`/`KS`/`HRR`) are also priced — single-leg, off Odds API per-event prop markets (`mlb_player_props`), player matched by normalized market-title name; HR props (`KXMLBHR`) are excluded (Odds API offers them one-sided, can't devig). Standalone process; writes `kalshi_mlb_mm/kalshi_mlb_mm.duckdb`. v1 is a measurement phase (5% quoted margin vs. realized adverse-selection cost). See `kalshi_mlb_mm/README.md`.
