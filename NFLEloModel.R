...
model {
  // Priors
  rating ~ normal(1500, 50);          // Team rating prior
...
win_total_data <- read.csv("Clean Win Totals - Sheet1.csv") %>%
  mutate(NFLfastR.Abbreviation = ifelse(NFLfastR.Abbreviation == "LAR", "LA", NFLfastR.Abbreviation))
...
traceplot(fd_win_fit, pars = c("team_wins[1]", "team_wins[2]"))
print(dk_win_fit)
fut_win <- data.frame(team = win_total_data$NFLfastR.Abbreviation,
                      fd_wins = summary(fd_win_fit, pars = c("team_wins"))$summary[,"mean"],
                      dk_wins = summary(dk_win_fit, pars = c("team_wins"))$summary[,"mean"]) %>%
  mutate(avg_wins = rowMeans(select(., fd_wins, dk_wins)))

# Normalize win totals
win_totals <- fut_win$avg_wins
...
elo_from_win_prob <- function(win_prob, avg_opponent_elo = 1500, home_field_adv = summary(fit, pars = c("home_adv"))$summary[,"mean"]) {
...
team_starting_elo <- data.frame(
  Team = win_total_data$NFLfastR.Abbreviation,
...
# fit Stan model with mcmc
nfl_fit <- stan(model_code = stan_model_code_nfl, data = nfl_data_list,
            iter = 2000, warmup = 500, chains = 4, seed = 123) 

# Print Stan fit
print(nfl_fit) #mean = estimate, se_mean = uncertainty of estimate, sd = variability in posterior, n_eff = number of independent samples, rhat = diagnose convergence, 1 means chains have converged

# Print selected parameter trace plots
traceplot(nfl_fit, pars = c("K", "home_adv", "rating[1]", "rating[2]"))

# Print team ELO ratings + home_adv & K parameter estimates
fit_summary <- summary(nfl_fit, pars = c("rating", "home_adv", "K"))$summary
...
fit_summary <- summary(nfl_fit, pars = "rating")$summary
rating_estimates <- fit_summary[, "mean"]
team_ratings <- data.frame(
  Team = names(team_ids),
  ELO_Rating = rating_estimates)
team_ratings <- team_ratings %>% arrange(desc(ELO_Rating))
posterior <- rstan::extract(nfl_fit)
...
predict_match_outcome <- function(home_team_name, away_team_name, location ="" ,n_simulations = 10000) {
  ...
  sim_outcomes <- replicate(n_simulations, {
    ...
    home_adv <- ifelse(location == "Neutral",0,sample(posterior$home_adv, 1))
...
test <- all %>% 
  ...
  mutate(home_team_win_prob = pmap_dbl(
    ...
    predict_match_outcome),
         exp_home_odds = 1/home_team_win_prob,
         exp_home_spread = (log((1/home_team_win_prob)-1)/0.143))
# Example matchup prediction
home_team <- "KC"
away_team <- "PHI"
win_prob_home <- predict_match_outcome(home_team, away_team, home_adv = "Neutral")
