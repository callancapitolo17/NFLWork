...
# Global options for kableExtra tables
...
playoffs <- c("DET","PHI","LA","TB","MIN","WAS","GB","KC","BUF","BAL","HOU","LAC","PIT","DEN")
```

  mutate(
    colour = ifelse(posteam %in% playoffs, NA, "b/w"),
    alpha = ifelse(posteam %in% playoffs, 0.9, 0.8)
  ) %>% 
...
# ggsave("OffSuccessLandscape.png", width = 14, height =10, dpi = "retina")
...
pbp_rp %>% 
  ...
  theme(legend.position = "top",
          ...
          panel.border = element_rect(colour = "white", fill = NA, size = 1))+
  geom_smooth(se = F, method = "lm", color = "white")
# ggsave("DefSuccessLandscape.png", width = 14, height =10, dpi = "retina")
```
...
# ggsave("OffBreakdown.png", width = 14, height =10, dpi = "retina")
...
pbp_rp %>% 
  ...
  geom_hline(yintercept = mean(pbp_rp$epa[pbp_rp$season == 2024 & pbp_rp$pass == 1],na.rm = T), linetype = "dashed",color = "white")+
  geom_vline(xintercept = mean(pbp_rp$epa[pbp_rp$season == 2024 & pbp_rp$rush == 1],na.rm = T), linetype = "dashed",color = "white")
# ggsave("DefBreakdown.png", width = 14, height =10, dpi = "retina")
...
#ggsave("DefPlayBreakout.png", width = 14, height =10, dpi = "retina")
...
pbp_rp %>%
  ...
  labs(y = "Offense", x = "EPA/Play Components", title = "Where are Offenses Generating Success From?", subtitle = "Sections represent different components of an offense's EPA/Play",caption = "@CapAnalytics7 | nflfastR")+
    theme(legend.position = "top",
        ...
        panel.border = element_rect(colour = "white", fill = NA, size = 1),
        panel.grid = element_blank())
# ggsave("OffPlayBreakout.png", width = 14, height =10, dpi = "retina")
...
#ggsave("OffEarlyvsLateEfficiency.png", width = 14, height =10, dpi = "retina")
...
pbp_rp %>%
  ...
  geom_hline(yintercept = mean(pbp_rp$epa[pbp_rp$down > 2],na.rm = T), linetype = "dashed",color = "white")+
  geom_vline(xintercept = mean(pbp_rp$epa[pbp_rp$down <= 2], na.rm = T), linetype = "dashed", color = "white")
#ggsave("DefEarlyvsLateEfficiency.png", width = 14, height =10, dpi = "retina")

```

## Explosives vs Negatives
```{r}
pbp_rp %>% 
  filter(season == year) %>% 
  # group_by(posteam) %>%
...
negatives %>% 
  ...
  labs(x = "Series Success With No Negative Plays", y = "Series Success With 1+ Negative Plays Prior to 4th Down",
       ...
       caption = "Series is defined as each time offense starts with 1st&10      @CapAnalytics7 | nflfastR" )+
  theme(legend.position = "top",
        legend.direction = "horizontal",
        legend.background = element_rect(fill = "white", color="white"),
        legend.title = element_blank(),
...
#   ) %>%
#     tab_options(
#     table.width = pct(100), # Set width percentage
#     table.font.size = px(10) # Adjust font size
#   )
# 
...
## Pass Over Exp vs EPA/Exped Pass Situation
```{r}
pbp_rp %>% 
  filter(season == year) %>% 
  group_by(posteam) %>% 
...
## Non Redzone vs Redzone Efficiency
```{r}
pbp_rp %>% 
  group_by(posteam) %>%
...
# ggsave("OffRedBreakOut.png", width = 14, height =10, dpi = "retina")
...
# ggsave("DefRedBreakOut.png", width = 14, height =10, dpi = "retina")
```
...
combined %>% 
  ...
  labs(x = "Away EPA/Play",
       y = "Home EPA/Play", title = "Home vs Away Offensive Efficiency",
       caption = "@CapAnalytics7 | nflfastR", subtitle = "Dashed Lines Represent League Average")+
  ...
  theme(legend.position = "none",
        ...
        axis.title = element_text(color = "white", size = 14),
        panel.border = element_rect(colour = "white", fill = NA, size = 1))+
  geom_mean_lines(aes(x0 = away_epa, y0 = home_epa), color = "white")
...
rushing_player %>%   
  ...
  labs(x = "Success Rate", y = "EPA/Rush", title = "Rushing Efficiency by Rusher", subtitle = "Minimum 50 Rushes",
       caption = "@CapAnalytics7 | nflfastR")+
  theme_bw()+
  geom_text_repel(
...
## Lead vs Trailing Offensive Efficiency
```{r}
leading <- pbp_rp %>% 
...
pbp %>% 
  ...
  ggplot(aes(x = net_epa_play, y = reorder(posteam,net_epa_play))) +
    ...
    labs(y = "", x = "Special Teams EPA/Play", title = "Special Teams", caption = "@CapAnalytics7 | nflfastR")+
    theme(legend.position = "top",
          legend.direction = "horizontal",
          legend.background = element_rect(fill = "white", color="white"),
          legend.title = element_blank(),
...
pbp %>% 
  ...
  left_join(pbp %>% 
              ...
              by = c("posteam" = "defteam")) %>% 
  mutate(net_epa_play = round((epa_def*-1+off_epa_play)/(def_plays+off_plays),5),
         total_epa = def_total_epa*-1+off_total_epa, total_wpa = off_total_wpa+def_total_wpa*-1) %>% 
  ggplot(aes(x = total_epa, y =total_wpa)) +
   geom_nfl_logos(aes(team_abbr = posteam), width = 0.045)+
    labs(y = "Total WPA", x = "Total EPA", title = "Special Teams Overview", caption = "@CapAnalytics7 | nflfastR")+
    theme(legend.position = "top",
          legend.direction = "horizontal",
          legend.background = element_rect(fill = "white", color="white"),
          legend.title = element_blank(),
  ...
  geom_mean_lines(aes(x0 = total_epa, y0 = total_wpa), color = "white")
ggsave("SpecialTeamsOverview.png", width = 14, height =10, dpi = "retina")
```


## Middle 8 Efficiency Landscape
```{r}
pbp_rp %>% 
  filter((half_seconds_remaining <=240 & qtr == 2) | (half_seconds_remaining >= 1560 & qtr == 3)) %>% 
  group_by(posteam) %>% 
...
pbp_rp %>% 
  ...
  geom_image(aes(image = team_logo_espn), size = 0.05, asp = 16/9)+
  labs(x = "Indoor", y = "Outdoor", title = "Outdoor vs Indoor Offensive Performance", 
       subtitle = "Teams Without Indoor or Outdoor Game Filtered Out")+
  geom_mean_lines(aes(x0 = indoor, y0 = outdoor), color = "white")+
  theme(legend.position = "top",
        legend.direction = "horizontal",
        legend.background = element_rect(fill = "white", color="white"),
        legend.title = element_blank(),
...
pbp_rp %>% 
  ...
  summarize(given_no_uc_pass_rate = mean(pass_oe[qb_location %in% c("S","U")],na.rm = T),
            ...
            given_no_uc_pass_rate = mean(pass[qb_location %in% c("S","U")],na.rm = T),
...
## Which Teams See the Biggest Shift in Schedule Difficulty?
```{r}
...
# ggsave("Interceptions.png", width = 14, height =10, dpi = "retina")
```
...
pbp_rp %>% 
  group_by(id) %>% 
  summarize(name = max(name),posteam = max(posteam), wpa = mean(wpa,na.rm =T), epa = mean(epa,na.rm = T), count = sum(pass,na.rm = T)) %>% 
  filter(count >100) %>% 
  ...
  labs(x = "WPA/Play",
       y = "EPA/Play", title = "Which QB Should be at the Forefront of the MVP Conversation?",
...
# ggsave("NFLMVP.png", width = 14, height =10, dpi = "retina")  
...
pbp %>% 
  ...
  mutate(bin_fg = ifelse(is.na(field_goal_result), NA, ifelse(field_goal_result == "made",1,0)),
         fg_oe = bin_fg- fg_prob,
         bin_xp = ifelse(is.na(extra_point_result), NA, ifelse(extra_point_result == "good",1,0)),
         xp_oe = bin_xp- extra_point_prob) %>% 
  group_by(kicker_player_id) %>% 
  summarize(name = first(kicker_player_name),team = first(posteam),xp_oe = mean(xp_oe,na.rm = T), fg_oe = mean(fg_oe,na.rm = T), count = n()) %>% 
  filter(count>=25) %>% 
  ggplot(aes(x = xp_oe, y = fg_oe))+
  geom_nfl_logos(aes(team_abbr = team), width = 0.025, alpha = 0.8)+
  geom_text_repel(
    aes(label = name),
    box.padding = 0.05,  # adjust this value for padding around the labels
    point.padding = 0.1,  # adjust this value for padding around the points
  ...
  labs(x = "Extra Point Over Expectation",
       y = "Field Goal Over Expectation", title = "Evaluating Kicker Performance",
       caption = "@CapAnalytics7 | nflfastR", subtitle = "Minimum 25 Attempts")+
  theme(legend.position = "top",
        legend.direction = "horizontal",
        legend.background = element_rect(fill = "white", color="white"),
        legend.title = element_blank(),
        ...
        panel.border = element_rect(colour = "white", fill = NA, size = 1),
        panel.grid = element_blank())+
  geom_mean_lines(aes(x0 = xp_oe, y0 = fg_oe), color = "white")
# ggsave("KickerPerformance.png", width = 14, height =10, dpi = "retina")
```
