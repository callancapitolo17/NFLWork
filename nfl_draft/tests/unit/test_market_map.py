...
def test_outright_group_key_top_n_range():
    # top_N_range rows bucket together so quarantine can pool-normalize
    # (devig_pool, sum-to-N) rather than treating the market as a 1-winner
    # outright. The group key is ``top_<N>`` so the pool size parses out.
    ...
    assert outright_group_key("top_32_range", "top_32_will-johnson") == "top_32"
