...
`cbb_odds`, `college_baseball_odds` (17-column standard schema + `draw_ml`
+ `idgm` = 19 cols total for `mlb_odds`; game timing is
`game_start_time TIMESTAMPTZ` (UTC), which replaces the legacy
`game_date VARCHAR` + `game_time VARCHAR` pair. The scraper converts WZ's
EDT response timestamps to UTC at write-time via
