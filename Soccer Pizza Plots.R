...
library(ggplot2)
library(ggtext)
player_comps<- function(player1,player2,league1 = "Last 365 Days Men's Big 5 Leagues, UCL, UEL",league2 = "Last 365 Days Men's Big 5 Leagues, UCL, UEL"){

data1 <- fb_player_scouting_report(pull(player_dictionary_mapping() %>% filter(PlayerFBref  == player1) %>% select(UrlFBref)%>% slice(1)), pos_versus = "primary", time_pause =  10)
data2 <- fb_player_scouting_report(pull(player_dictionary_mapping() %>% filter(PlayerFBref  == player2) %>% select(UrlFBref) %>% slice(1)), pos_versus = "primary",time_pause =  10)
...
comp_chart <- plot_pizza(data = data, type = "comparison", template = "custom",
  ...
  scale_x_discrete(limits = strikers,labels = function(x) stringr::str_wrap(x, width = 8))
comp_chart
ggsave("image.png", width = 4200, height = 2800, units = "px") 
print(comp_chart)
}
player_comps("Milos Kerkez", "Andrew Robertson")

mu25+mu26+plot_layout(ncol = 1)
